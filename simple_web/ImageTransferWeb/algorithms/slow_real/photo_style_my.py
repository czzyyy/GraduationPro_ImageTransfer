...
def load_seg(ima_seg):
    ...
    color_masks = []
    for i in range(10):
        color_masks.append(tf.expand_dims(tf.expand_dims(tf.constant(_extract_mask(ima_seg, i)), 0), -1))
    # add a left one
    mask_left = (ima_seg[:, :] >= 150).astype(np.uint8)
    color_masks.append(tf.expand_dims(tf.expand_dims(tf.constant(mask_left.astype(np.float32)), 0), -1))
    return color_masks
...
def train(content_path, style_path, save_path, content_seg_path, style_seg_path):
    ...
    with tf.name_scope('load_segs'):
        ...
        style_seg = np.array(utils.load_image_resize(style_seg_path, style_height, style_width)
                             , dtype=np.float32)  # [0-1]
        content_masks = load_seg(content_seg * 255.0)
